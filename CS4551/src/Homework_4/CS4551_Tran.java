...
public class CS4551_Tran
{
	public static boolean MainMenu(Scanner input)
	{
		...
		if (option.equals("1"))
		{
			MotionCompensationSetup(input);
		}
		else if (option.equals("2"))
		{
			RemovingMovingObjects(input);
		}
	...
	public static void MotionCompensationSetup(Scanner input)
	{
...
//			pS = "16";
	...
	public static void RemovingMovingObjects(Scanner input)
	{
		int targetFrame = -1;
		do
		{
...
//			pS = "16";
	...
	public static void MovingObjectDetectionandRemoval(Image targetImage, Image referenceImage, int n, int p)
	{
		...
		BlockBasedMotionCompensation(targetImage, referenceImage, new Image(targetImage.getW(), targetImage.getH()),
		...
		Image closestStaticReplacementImage = 
				ClosestStaticMacroBlockReplacement(targetImage, detectedMacroBlockArea, n);
		Image overlayStaticReplacementImage = 
				OverlayStaticMacroBlockReplacement(targetImage, detectedMacroBlockArea, n);
	...
	public static Image DetectionMacroBlock(Image targetImage, List<Vector2i> detectedMacroBlockArea, int n)
	{
		...
		for (Vector2i macroBlockVector : detectedMacroBlockArea)
		{
			for (int y = 0; y < n; y++)
			{
				...
				targetPixelRGB[0] = 255;
				targetPixelRGB[1] = 0;
				targetPixelRGB[2] = 0;

				detectedImage.setPixel((macroBlockVector.getX() * n) + 0, (macroBlockVector.getY() * n) + y,
						targetPixelRGB);
				
				detectedImage.setPixel((macroBlockVector.getX() * n) + n - 1, (macroBlockVector.getY() * n) + y,
						targetPixelRGB);
			}
			...
			for (int x = 0; x < n; x++)
			{
				...
				detectedImage.setPixel((macroBlockVector.getX() * n) + x, (macroBlockVector.getY() * n) + 0,
						targetPixelRGB);
				
				detectedImage.setPixel((macroBlockVector.getX() * n) + x, (macroBlockVector.getY() * n) + n - 1,
						targetPixelRGB);
			}
		}
	...
	public static Image OverlayStaticMacroBlockReplacement(Image targetImage, List<Vector2i> detectedMacroBlockArea,
			int n)
	{
		...
		for (Vector2i macroBlockVector : detectedMacroBlockArea)
		{
			for (int y = 0; y < n; y++)
			{
				for (int x = 0; x < n; x++)
				{
					...
					overlayImage.getPixel((macroBlockVector.getX() * n) + x, (macroBlockVector.getY() * n) + y,
							copyingPixelRGB);

					overlayStaticReplacementImage.setPixel((macroBlockVector.getX() * n) + x,
							(macroBlockVector.getY() * n) + y, copyingPixelRGB);
				}
			}
		}
	...
	public static Image ClosestStaticMacroBlockReplacement(Image targetImage, List<Vector2i> detectedMacroBlockArea,
			int n)
	{
		...
		for (Vector2i macroBlockVector : detectedMacroBlockArea)
		{
			...
			for (int i = 1; !hasReplace; i++)
			{
				...
				if (top < 0 && bottom >= closestStaticReplacementImage.getH() / n && left < 0
						&& right >= closestStaticReplacementImage.getH() / n)
					break;
	...
	public static void ReplaceMacroBlock(Image targetImage, Vector2i macroBlockVector,
			Vector2i copyMacroBlockOffsetVector, int n)
	{
		for (int y = 0; y < n; y++)
		{
			for (int x = 0; x < n; x++)
			{
				...
				targetImage.getPixel((copyMacroBlockOffsetVector.getX() * n) + x,
						(copyMacroBlockOffsetVector.getY() * n) + y, copyingPixelRGB);

				targetImage.setPixel((macroBlockVector.getX() * n) + x, (macroBlockVector.getY() * n) + y,
						copyingPixelRGB);
			}
		}
	}
			...
			Vector2i[][] motionVectorList, int n, int p)
	{
		...
		for (int imageBlockY = 0; imageBlockY < motionVectorList.length; imageBlockY++)
		{
			for (int imageBlockX = 0; imageBlockX < motionVectorList[0].length; imageBlockX++)
			{
					...
					for (int xOffset = 0; xOffset < (p * 2) + 1; xOffset++)
					{
						...
						for (int y = 0; y < n; y++)
						{
							...
							for (int x = 0; x < n; x++)
							{
								...
								if (referencePixelY < 0 || referencePixelY >= referenceImage.getH()
										|| referencePixelX < 0 || referencePixelX >= referenceImage.getW())
								{
									referencePixelRGB[0] = 0;
									referencePixelRGB[1] = 0;
									referencePixelRGB[2] = 0;
								}
								else
								{
									referenceImage.getPixel(referencePixelX, referencePixelY, referencePixelRGB);
								}

								targetImage.getPixel(targetPixelX, targetPixelY, targetPixelRGB);

								int targetGrayPixel = (int) Math.round(0.299 * targetPixelRGB[0]
										+ 0.587 * targetPixelRGB[1] + 0.114 * targetPixelRGB[2]);
								int referenceGrayPixel = (int) Math.round(0.299 * referencePixelRGB[0]
										+ 0.587 * referencePixelRGB[1] + 0.114 * referencePixelRGB[2]);
				...
				motionVectorList[imageBlockY][imageBlockX] = new Vector2i(targetBlockX - bestMatchedReferenceBlockX,
						targetBlockY - bestMatchedReferenceBlockY);
				...
				for (int y = 0; y < n; y++)
				{
					...
					for (int x = 0; x < n; x++)
					{
						...
						int targetGrayPixel = (int) Math.round(
								0.299 * targetPixelRGB[0] + 0.587 * targetPixelRGB[1] + 0.114 * targetPixelRGB[2]);
						int referenceGrayPixel = (int) Math.round(0.299 * referencePixelRGB[0]
								+ 0.587 * referencePixelRGB[1] + 0.114 * referencePixelRGB[2]);
		...
		for (int y = 0; y < residualImage.getH(); y++)
		{
			for (int x = 0; x < residualImage.getW(); x++)
			{
				...
				residualPixelRGB[0] = (int) (((double) (residualPixelRGB[0] - minError)
						/ (double) (maxError - minError)) * 255.0);
				residualPixelRGB[1] = (int) (((double) (residualPixelRGB[1] - minError)
						/ (double) (maxError - minError)) * 255.0);
				residualPixelRGB[2] = (int) (((double) (residualPixelRGB[2] - minError)
						/ (double) (maxError - minError)) * 255.0);
	...
	public static void CreateMotionVectorTextFile(Vector2i[][] motionVectorList, Image image, String targetName,
